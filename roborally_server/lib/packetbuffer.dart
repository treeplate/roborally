// This is hardlinked in roborally_client/lib/packetbuffer.dart and roborally_server/lib/packetbuffer.dart
...
class PacketBuffer {
  ...
  /// Reads the first 4 unread bytes, and returns them as an integer.
  /// Marks those bytes as read and decrements [available] by 4.
  int readUint32() {
    assert(available >= 4); // contract
    assert(_start < _buffer.first.length); // invariant
    int packetOffset = _cursor;
    for (Uint8List packet in _buffer) {
      if (packetOffset >= packet.length) {
      ...
      // We found the start of the int.
      Uint8List bytes;
      if (packetOffset + 4 <= packet.length) {
        bytes = packet;
      } else {
        // the int stradles the boundary
        _flatten();
      ...
      _cursor += 4;
      return bytes.buffer.asByteData().getUint32(packetOffset);
    }
    throw StateError('Unreachable.');
  }

  /// Returns the first unread byte, marks that byte as read and decrements [available] by 1.
  int readUint8() {
    assert(available >= 1); // contract
    assert(_start < _buffer.first.length); // invariant
    int packetOffset = _cursor;
    for (Uint8List packet in _buffer) {
      if (packetOffset >= packet.length) {
      ...
      _cursor += 1;
      return packet.buffer.asByteData().getUint8(packetOffset);
    }
    throw StateError('Unreachable.');
  }
